...
\subsection{Xử lý dữ liệu khuyết với Phương pháp gán đa biến bằng phương trình chuỗi (MICE - Multivariate imputation by chained equations)~\cite{mice}}
...
\end{longtable}
\subsection{Trọng số của bằng chứng (WOE - Weight Of Envidence) và Giá trị thông tin (IV - Information Value)~\cite{woeniv}}
...
\end{itemize}
\subsection{Xử lý dữ liệu ngoại biên (outliers) với Rừng cô lập (Isolation Forest)~\cite{if}}
...
\subsubsection{Độ nhọn (Kurtosis)~\cite{kurtosis}}
...
\end{enumerate}
\subsubsection{Độ lệch (Skewness)~\cite{skew}}
...
\subsection{Xử lý dữ liệu mất cân bằng trong bài toán phân loại~\cite{imbalance}}
